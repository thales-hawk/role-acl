import { AccessControl, getValueByPath } from "../src";
...
describe("Test Suite: Access Control", function () {
  ...
  it("should add grants from flat list (db) with extendRoles", async function () {
    ...
    expect(
      ac.can("user").execute("create").sync().on("video").granted
    ).toBeTruthy();
    expect(
      ac.can("editor").execute("create").sync().on("post").granted
    ).toBeTruthy();
    expect(
      ac.can("editor").execute("create").sync().on("video").granted
    ).toBeTruthy();
    expect(
      ac.can("editor").execute("delete").sync().on("post").granted
    ).toBeTruthy();
  });
  ...
  it("should grant access and check permissions", async function () {
    ...
    expect(
      (await ac.can("user").execute("create").on("photo")).attributes
    ).toEqual(attrs);

    // grant multiple roles the same permission for the same resource
    ac.grant(["user", "admin"]).execute("read").on("photo", attrs);
    expect((await ac.can("user").execute("read").on("photo")).granted).toEqual(
      true
    );
    expect((await ac.can("admin").execute("read").on("photo")).granted).toEqual(
      true
    );

    ac.grant("user").execute("update").on("photo", attrs);
    expect(
      (await ac.can("user").execute("update").on("photo")).attributes
    ).toEqual(attrs);

    ac.grant("user").execute("delete").on("photo", attrs);
    expect(
      (await ac.can("user").execute("delete").on("photo")).attributes
    ).toEqual(attrs);
  });
  ...
  it("should grant access and check permissions synchronously", function () {
    ...
    ac.grant("user").execute("create").on("photo", attrs);
    expect(
      ac.can("user").execute("create").sync().on("photo").attributes
    ).toEqual(attrs);

    // grant multiple roles the same permission for the same resource
    ac.grant(["user", "admin"]).execute("read").on("photo", attrs);
    expect(ac.can("user").execute("read").sync().on("photo").granted).toEqual(
      true
    );
    expect(ac.can("admin").execute("read").sync().on("photo").granted).toEqual(
      true
    );

    ac.grant("user").execute("update").on("photo", attrs);
    expect(
      ac.can("user").execute("update").sync().on("photo").attributes
    ).toEqual(attrs);

    ac.grant("user").execute("delete").on("photo", attrs);
    expect(
      ac.can("user").execute("delete").sync().on("photo").attributes
    ).toEqual(attrs);
  });
  ...
  it("should grant access and check permissions for wildcard resources", async function () {
    ...
    expect(
      (await ac.can("user1").execute("create").on("photo")).granted
    ).toEqual(false);
    expect(
      (await ac.can("user1").execute("create").on("video")).granted
    ).toEqual(true);
    ac.grant("user2").execute("create").on(["photo", "video"], attrs);
    expect(
      (await ac.can("user2").execute("create").on("photo")).granted
    ).toEqual(true);
    expect(
      (await ac.can("user2").execute("create").on("video")).granted
    ).toEqual(true);
    ac.grant("user3").execute("create").on(["!photo"], attrs);
    expect(
      (await ac.can("user3").execute("create").on("photo")).granted
    ).toEqual(false);
    expect(
      (await ac.can("user3").execute("create").on("video")).granted
    ).toEqual(true);
  });
  ...
  it("should grant access and check permissions for wildcard resources synchronously", function () {
    ...
    ac.grant("user1").execute("create").on("!photo", attrs);
    expect(
      ac.can("user1").execute("create").sync().on("photo").granted
    ).toEqual(false);
    expect(
      ac.can("user1").execute("create").sync().on("video").granted
    ).toEqual(true);
    ac.grant("user2").execute("create").on(["photo", "video"], attrs);
    expect(
      ac.can("user2").execute("create").sync().on("photo").granted
    ).toEqual(true);
    expect(
      ac.can("user2").execute("create").sync().on("video").granted
    ).toEqual(true);
    ac.grant("user3").execute("create").on(["!photo"], attrs);
    expect(
      ac.can("user3").execute("create").sync().on("photo").granted
    ).toEqual(false);
    expect(
      ac.can("user3").execute("create").sync().on("video").granted
    ).toEqual(true);
  });
  ...
  it("should grant access and check permissions for wildcard actions", async function () {
    ...
    expect(
      (await ac.can("user1").execute("update").on("photo")).granted
    ).toEqual(true);
    ...
    expect(
      (await ac.can("user2").execute("update").on("photo")).granted
    ).toEqual(true);
    expect(
      (await ac.can("user2").execute("create").on("photo")).granted
    ).toEqual(true);
    ...
    ac.grant("user3").execute(["*", "!create"]).on(["photo"], attrs);
    expect(
      (await ac.can("user3").execute("update").on("photo")).granted
    ).toEqual(true);
    expect(
      (await ac.can("user3").execute("create").on("photo")).granted
    ).toEqual(false);
  ...
  it("should grant access and check permissions for wildcard actions synchronously", function () {
    ...
    expect(
      ac.can("user1").execute("update").sync().on("photo").granted
    ).toEqual(true);
    ...
    ac.grant("user2").execute(["create", "update"]).on(["photo"], attrs);
    expect(
      ac.can("user2").execute("update").sync().on("photo").granted
    ).toEqual(true);
    expect(
      ac.can("user2").execute("create").sync().on("photo").granted
    ).toEqual(true);
    ...
    ac.grant("user3").execute(["*", "!create"]).on(["photo"], attrs);
    expect(
      ac.can("user3").execute("update").sync().on("photo").granted
    ).toEqual(true);
    expect(
      ac.can("user3").execute("create").sync().on("photo").granted
    ).toEqual(false);
  ...
  it("should filter object properties", async function () {
    expect(
      AccessControl.filter({ status: "approved", id: 123 }, ["*", "!status"])
    ).toEqual({ id: 123 });
    expect(
      AccessControl.filter({ status: "approved", id: 123 }, ["*"])
    ).toEqual({ status: "approved", id: 123 });
  });
  ...
  it("should grant access with custom actions and check permissions", async function () {
    ...
    ac.grant("sports/editor")
      .execute("publish")
      .when(categorySportsCondition)
      .on("article", attrs);
    permission = await ac
      .can("sports/editor")
      .execute("publish")
      .with(categorySportsContext)
      .on("article");
    expect(permission.attributes).toEqual(attrs);
  ...
  it("should grant access with OR condition and check permissions", async function () {
    ...
    ac.grant("user")
      .condition({
        Fn: "OR",
        args: [categorySportsCondition, categoryPoliticsCondition],
      })
      .execute("create")
      .on("article");
    expect(
    ...
    expect(
      (
        await ac
          .can("user")
          .context(categoryPoliticsContext)
          .execute("create")
          .on("article")
      ).granted
    ).toEqual(true);
  ...
  it("should grant access with equals conditions with null values", async function () {
  ...
  it("should grant access with JSONPath context keys or values with EQUALS condition", async function () {
    ...
    expect(
      (
        await ac
          .can("user")
          .context({ owner: "dilip", requester: "dilip" })
          .execute("edit")
          .on("article")
      ).granted
    ).toEqual(true);
    expect(
      (
        await ac
          .can("user")
          .context({ owner: "tensult", requester: "dilip" })
          .execute("edit")
          .on("article")
      ).granted
    ).toEqual(false);
    ...
    expect(
      (
        await ac
          .can("user")
          .context({ owner: "dilip", request: { initiator: "dilip" } })
          .execute("edit")
          .on("article")
      ).granted
    ).toEqual(true);
    expect(
      (
        await ac
          .can("user")
          .context({ owner: "tensult", request: { initiator: "dilip" } })
          .execute("edit")
          .on("article")
      ).granted
    ).toEqual(false);
  ...
  it("should support initializing ACL with custom named functions", async function () {
    ...
    const acUsingObj = new AccessControl(
      conditionalGrantObjectWithCustomNamedFunction.grants,
      conditionalGrantObjectWithCustomNamedFunction.customConditionFunctions
    );
    ...
    expect(
      (
        await acUsingObj
          .can("user")
          .context(customContextNotAllowed)
          .execute("edit")
          .on("profile")
      ).granted
    ).toEqual(false);
  ...
  it("should support registering custom named functions", async function () {
    let customConditionFunctions = {
      isOwner: (context: any): Promise<boolean> => {
        return new Promise<boolean>((resolve) => {
          setTimeout(() => {
            resolve(context.loginUserId === context.resourceProfileId);
          }, 200);
    ...
    ac.grant("user").condition("custom:isOwner").execute("edit").on("profile");
    expect(
      (
        await ac
          .can("user")
  ...
  it("should support registering custom named functions as condition object", async function () {
    ...
    ac.grant("user")
      .condition({ Fn: "custom:gte", args: { level: 2 } })
      .execute("comment")
      .on("article");
    expect(
      (
  ...
  it("should have valid basic custom condition example in the docs", async function () {
  ...
  it("should have valid no arguments custom condition example in the docs", async function () {
    const myConditions = {
    ...
    ac.grant("user")
      .condition("custom:isArticleOwner")
      .execute("delete")
      .on("article");
    ac.grant("user")
      .condition("custom:isArticleOwner")
    ...
    const permission = await ac
      .can("user")
      .context({ loginUserId: 1, articleOwnerId: 1 })
      .execute("update")
      // .sync()
      .on("article");
  ...
  it("should have valid async custom condition example in the docs", async function () {
    const asyncCheckResourceForUser = (resource, loginUserId, record) =>
      new Promise<boolean>((resolve) => {
        resolve(
          resource === "article" && loginUserId === 1 && record.id === 10
        );
      });
    ...
    const myConditions = {
      isResourceOwner: async (context, args) => {
        ...
        return asyncCheckResourceForUser(
          resource,
          loginUserId,
          context[resource]
        );
    ...
    expect(
      (
        await ac
          .can("user")
          .context({ loginUserId: 1, article: { id: 10 } })
          .execute("update")
          .on("article")
      ).granted
    ).toBe(true);
    expect(
      (
        await ac
          .can("user")
          .context({ loginUserId: 2, article: { id: 10 } })
          .execute("update")
          .on("article")
      ).granted
    ).toBe(false);
    ...
    const myPolicy = {
      ...
      myConditions: {
        ...
        isResourceOwner: (context, { resource }) => {
          ...
          return (
            getValueByPath(context, `$.${resource}.owner`) ===
            getValueByPath(context, "$.user.id")
          );
        },
      },
    };
  ...
  it("should validate custom named functions as condition string", async function () {
    const ac = new AccessControl();
    expect(() =>
      ac.grant("user").condition("custom:gte").execute("comment").on("article")
    ).toThrow();
  });
  ...
  it("should support initializing ACL when grants has custom functions", async function () {
    // Using object
    const acUsingObj = new AccessControl(
      conditionalGrantObjectWithCustomAsyncFunction
    );
    expect(
      (
        await acUsingObj
    ...
    // Using array
    const acUsingArray = new AccessControl(
      conditionalGrantArrayWithCustomAsyncFunction
    );
    expect(
      (
        await acUsingArray
  ...
  it("should support initializing ACL when grants has custom functions synchronously", async function () {
    // Using object
    const acUsingObj = new AccessControl(
      conditionalGrantObjectWithCustomSyncFunction
    );

    const p1 = await acUsingObj
      .can("sports/custom")
      .context(customContextAllowed)
      .execute("create")
      // .sync()
    ...
    const p2 = await acUsingObj
      .can("sports/custom")
      .context(customContextNotAllowed)
      .execute("edit")
      // .sync()
      .on("profile");
    expect(p1.granted).toEqual(true);
    expect(p2.granted).toEqual(false);

    // Using array
    const acUsingArray = new AccessControl(
      conditionalGrantArrayWithCustomSyncFunction
    );

    const p3 = await acUsingArray
      .can("sports/custom")
      .context(customContextAllowed)
      .execute("create")
      .on("profile");
    expect(p3.granted).toEqual(true);
    const p4 = await acUsingArray
      .can("sports/custom")
      .context(customContextNotAllowed)
      .execute("edit")
      .on("profile");
    expect(p4.granted).toEqual(false);
  });
  ...
  it("should stringfy and restore ACL with sync custom condition function", async function () {
    const ac = this.ac;
    ...
    const p1 = await newAC
      .can("user")
      .context(categorySportsContext)
      .execute("create")
      .sync()
      .on("article");
    const p2 = await newAC
      .can("user")
      .context(categoryPoliticsContext)
      .execute("create")
      .sync()
      .on("article");
    const p3 = await newAC
      .can("user")
      .context({ category: "tech" })
      .execute("create")
      .sync()
  ...
  it("should grant access with starts with condition on single value and check permissions synchronously", function () {
    ...
    expect(
      ac
        .can("user")
        .context({ tags: "sports", category: "sports" })
        .execute("create")
        .sync()
        .on("article").granted
    ).toEqual(true);
  ...
  it("should grant access with starts with condition with list value and check permissions", async function () {
    ...
    expect(
      (
        await ac
          .can("user")
          .context({ tags: "sports", mainCategory: "sports" })
          .execute("create")
          .on("article")
      ).granted
    ).toEqual(true);
    expect(
      (
        await ac
          .can("user")
          .context({ tags: "politics", subCategory: "politics" })
          .execute("create")
          .on("article")
      ).granted
    ).toEqual(true);
  ...
  it("should grant access to attribute based on conditions", async function () {
    ...
    ac.grant("user")
      .condition(categorySportsCondition)
      .execute("create")
      .on("article", sportsAttrs);
    ac.grant("user")
      .condition(categoryPoliticsCondition)
      .attributes(politicsAttrs)
      .execute("create")
      .on("article");
    expect(
      (
  ...
  it("should add conditional grants with list and check permissions", async function () {
    ...
    expect(
      (await ac.can("sports/writer").execute("create").on("article")).granted
    ).toEqual(false);
  });
  ...
  it("should add conditional grants with list and check permissions synchronously", function () {
    ...
    // should fail when context is not passed
    expect(
      ac.can("sports/writer").execute("create").sync().on("article").granted
    ).toEqual(false);
  });
  ...
  it("should chain grant methods and check permissions", async function () {
    ...
    ac.grant("superadmin")
      ...
      .on("profile", attrs)
      .execute("create")
      .on("video", []) // no attributes allowed
      .execute("create")
      .on("photo"); // all attributes allowed

    expect(
      (await ac.can("superadmin").execute("create").on("profile")).granted
    ).toEqual(true);
    expect(
      (await ac.can("superadmin").execute("read").on("profile")).granted
    ).toEqual(true);
    expect(
      (await ac.can("superadmin").execute("create").on("video")).granted
    ).toEqual(false);
    expect(
      (await ac.can("superadmin").execute("create").on("photo")).granted
    ).toEqual(true);
  });
  ...
  it("should chain grant methods and check permissions synchronously", function () {
    ...
    expect(
      ac.can("superadmin").execute("create").sync().on("profile").granted
    ).toEqual(true);
    expect(
      ac.can("superadmin").execute("read").sync().on("profile").granted
    ).toEqual(true);
    expect(
      ac.can("superadmin").execute("create").sync().on("video").granted
    ).toEqual(false);
    expect(
      ac.can("superadmin").execute("create").sync().on("photo").granted
    ).toEqual(true);
  });
  ...
  it("should grant access via object and check permissions", async function () {
    ...
    expect(
      (await ac.can("moderator").execute("create").on("post")).granted
    ).toEqual(true);
    expect(
      (await ac.can("moderator").execute("read").on("news")).granted
    ).toEqual(true);
    expect(
      (await ac.can("moderator").execute("update").on("book")).granted
    ).toEqual(true);

    // should overwrite already defined action in o1 object
    ac.grant(o1).execute("read").on();
    expect(
      (await ac.can("moderator").execute("read").on("post")).granted
    ).toEqual(true);

    // non-set action (update:own)
    expect(
      (await ac.can("moderator").execute("update").on("news")).granted
    ).toEqual(false);
    // non-existent resource
    expect(
      (await ac.can("moderator").execute("create").on("foo")).granted
    ).toEqual(false);
  });
  ...
  it("should grant access via object and check permissions synchronously", function () {
    ...
    expect(
      ac.can("moderator").execute("create").sync().on("post").granted
    ).toEqual(true);
    expect(
      ac.can("moderator").execute("read").sync().on("news").granted
    ).toEqual(true);
    expect(
      ac.can("moderator").execute("update").sync().on("book").granted
    ).toEqual(true);

    // should overwrite already defined action in o1 object
    ac.grant(o1).execute("read").on();
    expect(
      ac.can("moderator").execute("read").sync().on("post").granted
    ).toEqual(true);

    // non-set action (update:own)
    expect(
      ac.can("moderator").execute("update").sync().on("news").granted
    ).toEqual(false);
    // non-existent resource
    expect(
      ac.can("moderator").execute("create").sync().on("foo").granted
    ).toEqual(false);
  });
  ...
  it("should skip conditions when skipConditions is used", async function () {
    ...
    expect(
      (await ac.can("user").execute("create").on("article", true)).granted
    ).toEqual(true); // conditions skipped
    expect(
      (await ac.can("user").execute("create").on("article", false)).granted
    ).toEqual(false); // context not set
    expect(
      (await ac.can("user").execute("create").on("article")).granted
    ).toEqual(false); // context not set
    expect(
      (
        await ac.permission({
  ...
  it("should skip conditions when skipConditions is used synchronously", function () {
    ...
    expect(
      ac.can("user").execute("create").sync().on("article", true).granted
    ).toEqual(true); // conditions skipped
    expect(
      ac.can("user").execute("create").sync().on("article", false).granted
    ).toEqual(false); // context not set
    expect(
      ac.can("user").execute("create").sync().on("article").granted
    ).toEqual(false); // context not set
    expect(
      ac.permissionSync({ role: "user", action: "create", resource: "article" })
        .granted
    ).toEqual(false); // context not set
    // context not set and conditions skipped
    expect(
  ...
  it("should return allowed resources for given roles", async function () {
    ...
    expect((await ac.allowedResources({ role: "user" })).sort()).toEqual([
      "article",
      "image",
    ]);
    expect(
      (
        await ac.allowedResources({
          role: "user",
          context: categorySportsContext,
        })
      ).sort()
    ...
    expect((await ac.allowedResources({ role: "admin" })).sort()).toEqual([
      "article",
      "category",
      "image",
    ]);
    expect((await ac.allowedResources({ role: "owner" })).sort()).toEqual([
      "article",
      "category",
      "image",
      "video",
    ...
    expect(
      (await ac.allowedResources({ role: ["admin", "owner"] })).sort()
    ).toEqual(["article", "category", "image", "video"]);
  });
  ...
  it("should return allowed resources for given roles synchronously", function () {
    ...
    expect(ac.allowedResourcesSync({ role: "user" }).sort()).toEqual([
      "article",
      "image",
    ]);
    expect(
      ac
        .allowedResourcesSync({ role: "user", context: categorySportsContext })
        .sort()
    ).toEqual(["article", "image"]);
    expect(
      ac
        .allowedResourcesSync({
          role: "user",
          context: categoryPoliticsContext,
        })
        .sort()
    ).toEqual(["image"]);
    expect(ac.allowedResourcesSync({ role: "admin" }).sort()).toEqual([
      "article",
      "category",
      "image",
    ]);
    expect(ac.allowedResourcesSync({ role: "owner" }).sort()).toEqual([
      "article",
      "category",
      "image",
      "video",
    ...
    expect(
      ac.allowedResourcesSync({ role: ["admin", "owner"] }).sort()
    ).toEqual(["article", "category", "image", "video"]);
  });
  ...
  it("should return allowed actions for given roles and resource", async function () {
    ...
    expect(
      (await ac.allowedActions({ role: "user", resource: "article" })).sort()
    ).toEqual(["create"]);
    expect(
      (
        await ac.allowedActions({
          role: "user",
          resource: "article",
          context: categorySportsContext,
        })
    ...
    expect(
      (await ac.allowedActions({ role: "admin", resource: "category" })).sort()
    ).toEqual(["*"]);
    expect(
      (await ac.allowedActions({ role: "owner", resource: "video" })).sort()
    ).toEqual(["*"]);
  });
  ...
  it("should return allowed actions for given roles and resource synchronously", function () {
    ...
    expect(
      ac.allowedActionsSync({ role: "user", resource: "article" }).sort()
    ).toEqual(["create"]);
    expect(
      ac
        .allowedActionsSync({
    ...
    expect(
      ac
        .allowedActionsSync({ role: ["admin", "user"], resource: "article" })
        .sort()
    ).toEqual(["create", "delete"]);

    expect(
      ac.allowedActionsSync({ role: "admin", resource: "category" }).sort()
    ).toEqual(["*"]);
    expect(
      ac.allowedActionsSync({ role: "owner", resource: "video" }).sort()
    ).toEqual(["*"]);
  });
  ...
  it("should return allowing roles for given permission", async function () {
    ...
    expect(
      (await ac.allowingRoles({ resource: "image", action: "create" })).sort()
    ).toEqual(["admin", "owner", "sports/editor", "user"]);
    expect(
      (await ac.allowingRoles({ resource: "video", action: "create" })).sort()
    ).toEqual(["owner"]);
    expect(
      (
        await ac.allowingRoles({ resource: "category", action: "create" })
      ).sort()
    ).toEqual(["admin", "owner"]);
    expect(
      await ac.allowingRoles({ resource: "blog", action: "create" })
    ).toEqual([]);
    expect(
      await ac.allowingRoles({
        resource: "blog",
        action: "create",
        context: categoryPoliticsContext,
      })
    ...
    ).toEqual(["sports/editor", "sports/writer"]);
  ...
  it("should return allowing roles for given permission synchronously", function () {
    ...
    expect(
      ac.allowingRolesSync({ resource: "image", action: "create" }).sort()
    ).toEqual(["admin", "owner", "sports/editor", "user"]);
    expect(
      ac.allowingRolesSync({ resource: "video", action: "create" }).sort()
    ).toEqual(["owner"]);
    expect(
      ac.allowingRolesSync({ resource: "category", action: "create" }).sort()
    ).toEqual(["admin", "owner"]);
    expect(
      ac.allowingRolesSync({ resource: "blog", action: "create" })
    ).toEqual([]);
    expect(
      ac.allowingRolesSync({
        resource: "blog",
        action: "create",
        context: categoryPoliticsContext,
      })
  ...
  it("should grant access (variation, chained)", async function () {
    ...
    expect(
      (await ac.can("admin").execute("create").on("video")).granted
    ).toEqual(true);

    ac.grant("foo").execute("create").on("bar");
    expect((await ac.can("foo").execute("create").on("bar")).granted).toEqual(
      true
    );

    ac.grant("foo").execute("create").on("baz", []); // no attributes, actually denied instead of granted
    expect((await ac.can("foo").execute("create").on("baz")).granted).toEqual(
      false
    );
    ...
    ac.grant("qux")
      ...
      .on("resource1", []);
    expect(
      (await ac.can("qux").execute("create").on("resource1")).granted
    ).toEqual(true);
    expect(
      (await ac.can("qux").execute("update").on("resource2")).granted
    ).toEqual(true);
    expect(
      (await ac.can("qux").execute("read").on("resource1")).granted
    ).toEqual(true);
    expect(
      (await ac.can("qux").execute("delete").on("resource1")).granted
    ).toEqual(false);
    ...
    expect(
      (await ac.can("editor").execute("update").on("file1")).granted
    ).toEqual(true);
    expect(
      (await ac.can("editor").execute("update").on("file2")).granted
    ).toEqual(true);
    expect(
      (await ac.can("editor").execute("update").on("file3")).granted
    ).toEqual(true);
    ...
    ac.grant("editor")
      ...
      .on();
    expect(
      (await ac.can("editor").execute("read").on("fileX")).granted
    ).toEqual(true);
    expect(
      (await ac.can("editor").execute("create").on("fileX")).granted
    ).toEqual(true);
    expect(
      (await ac.can("editor").execute("update").on("fileY")).granted
    ).toEqual(true);
    expect(
      (await ac.can("editor").execute("delete").on("fileY")).granted
    ).toEqual(true);
  });
  ...
  it("should grant access (variation, chained) synchronously", function () {
    ...
    expect(
      ac.can("admin").execute("create").sync().on("video").granted
    ).toEqual(true);

    ac.grant("foo").execute("create").on("bar");
    expect(ac.can("foo").execute("create").sync().on("bar").granted).toEqual(
      true
    );

    ac.grant("foo").execute("create").on("baz", []); // no attributes, actually denied instead of granted
    expect(ac.can("foo").execute("create").sync().on("baz").granted).toEqual(
      false
    );
    ...
    ac.grant("qux")
      ...
      .on("resource1", []);
    expect(
      ac.can("qux").execute("create").sync().on("resource1").granted
    ).toEqual(true);
    expect(
      ac.can("qux").execute("update").sync().on("resource2").granted
    ).toEqual(true);
    expect(
      ac.can("qux").execute("read").sync().on("resource1").granted
    ).toEqual(true);
    expect(
      ac.can("qux").execute("delete").sync().on("resource1").granted
    ).toEqual(false);
    ...
    ac.grant().role("editor").resource("file3").execute("update").on();
    expect(
      ac.can("editor").execute("update").sync().on("file1").granted
    ).toEqual(true);
    expect(
      ac.can("editor").execute("update").sync().on("file2").granted
    ).toEqual(true);
    expect(
      ac.can("editor").execute("update").sync().on("file3").granted
    ).toEqual(true);
    ...
    ac.grant("editor")
      ...
      .execute("delete")
      .on();
    expect(ac.can("editor").execute("read").sync().on("fileX").granted).toEqual(
      true
    );
    expect(
      ac.can("editor").execute("create").sync().on("fileX").granted
    ).toEqual(true);
    expect(
      ac.can("editor").execute("update").sync().on("fileY").granted
    ).toEqual(true);
    expect(
      ac.can("editor").execute("delete").sync().on("fileY").granted
    ).toEqual(true);
  });
  ...
  it("should switch-chain grant roles", async function () {
    ...
    expect((await ac.can("r1").execute("create").on("a")).granted).toEqual(
      true
    );
    expect((await ac.can("r1").execute("update").on("c")).granted).toEqual(
      true
    );
    expect((await ac.can("r2").execute("create").on("b")).granted).toEqual(
      true
    );
  ...
  it("should grant comma/semi-colon separated roles", async function () {
    ...
    ac.grant("role2; role3, editor; viewer, agent")
      .execute("create")
      .on("book");
  ...
  it("permission should also return queried role(s) and resource", async function () {
    ...
    expect((await ac.can("bar").execute("create").on("baz")).granted).toEqual(
      true
    );
    // returned permission should provide queried role(s) as array
    expect((await ac.can("foo").execute("create").on("baz")).roles).toContain(
      "foo"
    );
    // returned permission should provide queried resource
    expect((await ac.can("foo").execute("create").on("baz")).resource).toEqual(
      "baz"
    );
  ...
  it("permission should also return queried role(s) and resource synchronously", function () {
    ...
    // also supporting comma/semi-colon separated roles
    ac.grant("foo, bar").execute("create").on("baz");
    expect(ac.can("bar").execute("create").sync().on("baz").granted).toEqual(
      true
    );
    // returned permission should provide queried role(s) as array
    expect(ac.can("foo").execute("create").sync().on("baz").roles).toContain(
      "foo"
    );
    // returned permission should provide queried resource
    expect(ac.can("foo").execute("create").sync().on("baz").resource).toEqual(
      "baz"
    );
    // create is execute("create").on. but above only returns the queried value, not the result.
  });
  ...
  it("should extend / remove roles", async function () {
    ...
    expect(ac.getGrants().onur.$extend["admin"]).toEqual({
      condition: undefined,
    });
    ...
    expect(Object.keys(ac.getGrants().onur.$extend).sort()).toEqual([
      "admin",
      "role2",
      "role3",
    ]);
    ...
    (await ac.grant("admin").extend(["viewer", "editor", "agent"]))
      .execute("read")
      .on("video");
    expect(Object.keys(ac.getGrants().admin.$extend).sort()).toEqual([
      "agent",
      "editor",
      "viewer",
    ]);
    (await ac.grant(["editor", "agent"]).extend(["role2", "role3"]))
      .execute("update")
      .on("photo");
    expect(Object.keys(ac.getGrants().editor.$extend).sort()).toEqual([
      "role2",
      "role3",
    ]);
  ...
  it("should extend / remove roles synchronously", function () {
    ...
    expect(Object.keys(ac.getGrants().admin.$extend)).toEqual(["editor"]);
    ac.grant("admin")
      .extendSync(["viewer", "editor", "agent"])
      .execute("read")
      .on("video");
    expect(Object.keys(ac.getGrants().admin.$extend).sort()).toEqual([
      "agent",
    ...
    ac.grant(["editor", "agent"])
      .extendSync(["role2", "role3"])
      .execute("update")
      .on("photo");
    expect(Object.keys(ac.getGrants().editor.$extend).sort()).toEqual([
      "role2",
    ...
    throwsError(() =>
      ac.grant(["admin2", "roleX"]).extendSync(["roleX", "admin3"])
    );
  });
  ...
  it("should extend roles and reflect the changes done to original role used synchronously", function () {
    ...
    expect(
      ac
        .can("editor")
        .context(categoryBusinessContext)
        .execute("create")
        .sync()
        .on("post").granted
    ).toEqual(true);
  ...
  it("should extend roles with conditions", async function () {
    ...
    expect(
      (await ac.can("editor").execute("create").on("post")).granted
    ).toEqual(true);
    expect(
      (
        await ac
  ...
  it("should support multi-level extension of roles when conditions used", async function () {
    ...
    ac.extendRole("sports-and-politics/editor", [
      "sports/editor",
      "politics/editor",
    ]);
    expect(
      (
        await ac
          .can("sports-and-politics/editor")
          .context(categorySportsContext)
          .execute("create")
          .on("post")
      ).granted
    ...
    ac.extendRole(
      "conditonal/sports-and-politics/editor",
      "sports-and-politics/editor",
      {
        Fn: "EQUALS",
        args: { status: "draft" },
      }
    ...
    expect(
      (
        await ac
          .can("conditonal/sports-and-politics/editor")
          .context({
            category: "sports",
            status: "draft",
          })
  ...
  it("should throw if grant objects are invalid", async function () {
    ...
    expect(
      (await ac.can("admin2").execute("create").on("post")).granted
    ).toEqual(true);
  });
  ...
  it("should throw if grant objects are invalid synchronously", function () {
    ...
    expect(() => ac.grant(o)).not.toThrow();
    expect(
      ac.can("admin2").execute("create").sync().on("post").granted
    ).toEqual(true);
  });
  ...
  it("should throw `AccessControlError` for invalid role", async function () {
    ...
    ac.setGrants(grantsObject);
    await promiseThrowsError(
      ac.can("invalid-role").execute("create").on("video"),
      "Role not found"
    );
  });
  ...
  it("should throw `AccessControlError` for invalid role synchronously", function () {
    ...
    ac.setGrants(grantsObject);
    throwsError(
      () => ac.can("invalid-role").execute("create").sync().on("video"),
      "Role not found"
    );
  });
  ...
  it("Check with multiple roles changes grant list (issue #2)", async function () {
    ...
    expect(
      (await ac.can(["admin"]).execute("update").on("video")).granted
    ).toEqual(true);
  ...
  it("Check with multiple roles changes grant list (issue #2) synchronously", function () {
    ...
    // Admin can update any video
    expect(
      ac.can(["admin"]).execute("update").sync().on("video").granted
    ).toEqual(true);
  ...
  it("should grant multiple roles and multiple resources", async function () {
    ...
    expect(
      (await ac.can("admin").execute("create").on("profile")).granted
    ).toEqual(true);
    expect(
      (await ac.can("admin").execute("create").on("video")).granted
    ).toEqual(true);
    expect(
      (await ac.can("user").execute("create").on("profile")).granted
    ).toEqual(true);
    expect(
      (await ac.can("user").execute("create").on("video")).granted
    ).toEqual(true);

    ac.grant("admin, user").execute("create").on("profile, video", "*,!id");
    expect(
      (await ac.can("admin").execute("create").on("profile")).attributes
    ).toEqual(["*"]);
    expect(
      (await ac.can("admin").execute("create").on("video")).attributes
    ).toEqual(["*"]);
    expect(
      (await ac.can("user").execute("create").on("profile")).attributes
    ).toEqual(["*"]);
    expect(
      (await ac.can("user").execute("create").on("video")).attributes
    ).toEqual(["*"]);

    expect(
      (await ac.can("user").execute("create").on("non-existent")).granted
    ).toEqual(false);
  });
  ...
  it("should grant multiple roles and multiple resources synchronously", function () {
    ...
    ac.grant("admin, user").execute("create").on("profile, video");
    expect(
      ac.can("admin").execute("create").sync().on("profile").granted
    ).toEqual(true);
    expect(
      ac.can("admin").execute("create").sync().on("video").granted
    ).toEqual(true);
    expect(
      ac.can("user").execute("create").sync().on("profile").granted
    ).toEqual(true);
    expect(ac.can("user").execute("create").sync().on("video").granted).toEqual(
      true
    );

    ac.grant("admin, user").execute("create").on("profile, video", "*,!id");
    expect(
      ac.can("admin").execute("create").sync().on("profile").attributes
    ).toEqual(["*"]);
    expect(
      ac.can("admin").execute("create").sync().on("video").attributes
    ).toEqual(["*"]);
    expect(
      ac.can("user").execute("create").sync().on("profile").attributes
    ).toEqual(["*"]);
    expect(
      ac.can("user").execute("create").sync().on("video").attributes
    ).toEqual(["*"]);

    expect(
      ac.can("user").execute("create").sync().on("non-existent").granted
    ).toEqual(false);
  });
});
