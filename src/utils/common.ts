...
export class CommonUtil {
  ...
  public static containsPromises(elements: any[]) {
    return elements.some((elm) => {
      return (
        elm && typeof elm.then === "function" && Promise.resolve(elm) == elm
      );
    });
  }
   ...
   */
  public static async getFlatRoles(
    grants: any,
    roles: string | string[],
    context?: any,
      ...
      if (roleItem.$extend) {
        ...
        if (skipConditions) {
          ...
          for (let extendedRoleName of Object.keys(roleItem.$extend)) {
            if (
              await ConditionUtil.evaluate(
                roleItem.$extend[extendedRoleName].condition,
                context
              )
            ) {
        ...
        arr = ArrayUtil.uniqConcat(
          arr,
          await this.getFlatRoles(
            grants,
            rolesMetCondition,
            context,
            skipConditions
  ...
  public static getFlatRolesSync(
    grants: any,
    roles: string | string[],
    context?: any,
    skipConditions?: boolean
    ...
    for (let roleName of roles) {
      ...
      if (roleItem.$extend) {
        ...
        if (skipConditions) {
          ...
          for (let extendedRoleName of Object.keys(roleItem.$extend)) {
            const conditionResult = ConditionUtil.evaluate(
              roleItem.$extend[extendedRoleName].condition,
              context
            );
            ...
            if (typeof conditionResult !== "boolean") {
              throw new AccessControlError(
                `Expected the condition function should return boolean, but returning ${conditionResult}`
              );
  ...
  public static normalizeQueryInfo(query: IQueryInfo): IQueryInfo {
    ...
    if (!ArrayUtil.isFilledStringArray(newQuery.role)) {
      throw new AccessControlError(
        `Invalid role(s): ${JSON.stringify(newQuery.role)}`
      );
    }
    ...
    if (newQuery.resource) {
      if (
        typeof newQuery.resource !== "string" ||
        newQuery.resource.trim() === ""
      ) {
        throw new AccessControlError(
          `Invalid resource: "${newQuery.resource}"`
        );
      }
    ...
    if (newQuery.action) {
      if (
        typeof newQuery.action !== "string" ||
        newQuery.action.trim() === ""
      ) {
  ...
  public static normalizeAccessInfo(access: IAccessInfo): IAccessInfo {
    ...
    if (!ArrayUtil.isFilledStringArray(newAccess.role)) {
      throw new AccessControlError(
        `Invalid role(s): ${JSON.stringify(newAccess.role)}`
      );
    }
    ...
    if (!ArrayUtil.isFilledStringArray(newAccess.resource)) {
      throw new AccessControlError(
        `Invalid resource(s): ${JSON.stringify(newAccess.resource)}`
      );
    }
    ...
    if (!ArrayUtil.isFilledStringArray(newAccess.action)) {
      throw new AccessControlError(
        `Invalid resource(s): ${JSON.stringify(newAccess.action)}`
      );
    }

    newAccess.attributes = !newAccess.attributes
      ? ["*"]
      : ArrayUtil.toStringArray(newAccess.attributes);
  ...
  public static async getUnionGrantsOfRoles(
    grants: any,
    query: IQueryInfo
  ): Promise<IAccessInfo[]> {
    if (!grants) {
    ...
    const roles: string[] = await this.getFlatRoles(
      grants,
      query.role,
      query.context,
      query.skipConditions
    );
  ...
  public static getUnionGrantsOfRolesSync(
    grants: any,
    query: IQueryInfo
  ): IAccessInfo[] {
    if (!grants) {
    ...
    // get roles and extended roles in a flat array
    const roles: string[] = this.getFlatRolesSync(
      grants,
      query.role,
      query.context,
      query.skipConditions
  ...
  public static async getUnionResourcesOfRoles(
    grants: any,
    query: IQueryInfo
  ): Promise<string[]> {
  ...
  public static getUnionResourcesOfRolesSync(
    grants: any,
    query: IQueryInfo
  ): string[] {
    query.skipConditions = query.skipConditions || !query.context;
  ...
  public static async getUnionActionsOfRoles(
    grants: any,
    query: IQueryInfo
  ): Promise<string[]> {
    query.skipConditions = query.skipConditions || !query.context;
    ...
    const matchingGrants = (
      await this.getUnionGrantsOfRoles(grants, query)
    ).filter((grant) => {
  ...
  public static getUnionActionsOfRolesSync(
    grants: any,
    query: IQueryInfo
  ): string[] {
    query.skipConditions = query.skipConditions || !query.context;

    const matchingGrants = this.getUnionGrantsOfRolesSync(grants, query).filter(
      (grant) => {
        return this.anyMatch(query.resource, grant.resource);
      }
    );
   ...
   */
  public static async getUnionAttrsOfRoles(
    grants: any,
    query: IQueryInfo
  ): Promise<string[]> {
    const matchingGrants = (
  ...
  public static getUnionAttrsOfRolesSync(
    grants: any,
    query: IQueryInfo
  ): string[] {
    const matchingGrants = this.getUnionGrantsOfRolesSync(grants, query).filter(
  ...
  public static async filterGrantsAllowing(
    grants: IAccessInfo[],
    query: IQueryInfo
  ): Promise<IAccessInfo[]> {
  ...
  public static filterGrantsAllowingSync(
    grants: IAccessInfo[],
    query: IQueryInfo
  ): IAccessInfo[] {
    if (query.skipConditions) {
      ...
      for (let grant of grants) {
        const conditionResult =
          query.skipConditions ||
          ConditionUtil.evaluate(grant.condition, query.context);
        if (typeof conditionResult !== "boolean") {
          throw new AccessControlError(
            `Expected the condition function should return boolean, but returning ${conditionResult}`
          );
  ...
  public static async areGrantsAllowing(
    grants: IAccessInfo[],
    query: IQueryInfo
  ): Promise<boolean> {
    if (!grants) {
    ...
    for (let grant of grants) {
      result =
        ...
        (this.anyMatch(query.resource, grant.resource) &&
          this.anyMatch(query.action, grant.action) &&
          (query.skipConditions ||
            (await ConditionUtil.evaluate(grant.condition, query.context))));
    }
    return result;
  }

  public static areGrantsAllowingSync(
    grants: IAccessInfo[],
    query: IQueryInfo
  ): boolean {
    if (!grants) {
  ...
  public static async areExtendingRolesAllowing(
    roleExtensionObject: any,
    allowingRoles: any,
    query: IQueryInfo
  ): Promise<boolean> {
    ...
    for (let roleName in roleExtensionObject) {
      result =
        ...
        (allowingRoles[roleName] &&
          (query.skipConditions ||
            (await ConditionUtil.evaluate(
              roleExtensionObject[roleName].condition,
              query.context
            ))));
    }
    return result;
  ...
  public static areExtendingRolesAllowingSync(
    roleExtensionObject: any,
    allowingRoles: any,
    query: IQueryInfo
  ): boolean {
    ...
    for (let roleName in roleExtensionObject) {
      ...
      result =
        result ||
        (allowingRoles[roleName] && (query.skipConditions || conditionResult));
    }
    return result;
  }

  public static async getAllowingRoles(
    grants: any,
    query: IQueryInfo
  ): Promise<string[]> {
    if (!grants) {
    ...
    for (let role of sortedRoles) {
      allowingRoles[role] =
        (await this.areGrantsAllowing(grants[role].grants, query)) ||
        (await this.areExtendingRolesAllowing(
          grants[role].$extend,
          allowingRoles,
          query
        ));
  ...
  public static getAllowingRolesSync(grants: any, query: IQueryInfo): string[] {
    ...
    for (let role of sortedRoles) {
      allowingRoles[role] =
        this.areGrantsAllowingSync(grants[role].grants, query) ||
        this.areExtendingRolesAllowingSync(
          grants[role].$extend,
          allowingRoles,
          query
        );
  ...
  public static extendRoleSync(
    ...
    if (!arrExtRoles)
      throw new AccessControlError(
        `Invalid extender role(s): ${JSON.stringify(extenderRoles)}`
      );
    let nonExistentExtRoles: string[] = this.getNonExistentRoles(
      grants,
      arrExtRoles
    );
    if (nonExistentExtRoles.length > 0) {
      throw new AccessControlError(
        `Cannot extend with non-existent role(s): "${nonExistentExtRoles.join(
          ", "
        )}"`
      );
    }
    ...
    if (!roles)
      throw new AccessControlError(`Invalid role(s): ${JSON.stringify(roles)}`);
    const allExtendingRoles = this.getFlatRolesSync(
      grants,
      arrExtRoles,
      null,
      true
    ...
    roles.forEach((role: string) => {
      if (allExtendingRoles.indexOf(role) >= 0) {
        throw new AccessControlError(
          `Attempted to extend role "${role}" by itself.`
        );
      }
      grants[role] = grants[role] || { score: 1 };
  ...
  public static matchesAllElement(
    values: any,
    predicateFn: (elm) => boolean
  ): boolean {
    values = ArrayUtil.toArray(values);
    return values.every(predicateFn);
  }

  public static matchesAnyElement(
    values: any,
    predicateFn: (elm) => boolean
  ): boolean {
    values = ArrayUtil.toArray(values);
